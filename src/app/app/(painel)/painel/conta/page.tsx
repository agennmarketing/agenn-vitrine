...
export default async function AccountPage() {
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-6">
      <PageHeader title="Conta" description="Gerencie seus dados e o acesso à sua conta." />

      {/* Cartão de identidade: a mesma inicial do cabeçalho do painel, em tamanho grande. */}
      <div className="flex items-center gap-4 rounded-card bg-deep px-5 py-4 text-deep-ink">
        <span
          aria-hidden="true"
        ...
        <div className="flex min-w-0 flex-col gap-0.5">
          <p className="truncate text-lg font-black leading-tight tracking-[-0.02em]">{displayName}</p>
          {name ? <p className="truncate text-sm font-bold text-deep-muted">{email}</p> : null}
        </div>
      </div>
