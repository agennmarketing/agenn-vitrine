import { CalendarDays, LayoutGrid, MessageCircle, Palette, Settings, Share2, ShoppingBag, Wrench } from 'lucide-react'
import type { VitrineType } from './vitrine-types'

/*
 * Seções do editor de cada tipo de vitrine (as abas do editor).
 * Serviços: a lista se chama "Serviços", há a Agenda e não há sacola. Produtos (e as antigas de comida) têm as duas.
 */
export function editorSections(type: VitrineType) {
  ...
  return [
    ...
    ...(servicos ? [{ slug: 'agenda', label: 'Agenda', Icon: CalendarDays }] : []),
